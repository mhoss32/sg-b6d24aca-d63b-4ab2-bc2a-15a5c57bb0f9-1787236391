...
const toBeConfig: Record<string, { color: string; bg: string; border: string; label: string; iconSvg: string }> = {
  time: {
    ...
    label: "Time Saving",
    iconSvg: '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#22d3ee" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',
  },
  gain: {
    color: "#c084fc",
...
function renderSynergyColumn(ratings: SynergyRating): string {
  ...
  const colors: Record<string, { text: string; border: string; bg: string }> = {
    ...
    None: { text: "#64748b", border: "rgba(100,116,139,0.25)", bg: "rgba(100,116,139,0.12)" },
  };
...
function renderHomePage(ibmB64: string, systemB64: string, changeB64: string, predictiveB64: string): string {
  ...
  // Build row placement: each use case placed in the earliest row where all its pillar columns are free
    ...
    <div style="max-width: 1280px; margin: 0 auto; padding: 0 24px 32px;">
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); grid-auto-rows: minmax(60px, auto); gap: 16px;">
        ...
        ${useCases.map((uc) => {
          ...
          if (!row || !span) return "";
          const leftColor = PILLARS.find((p) => p.id === ucPillars[0])?.color || "#E2E8F0";
          ...
          return `
          <div onclick="showPage('uc-${uc.id}')" style="cursor: pointer; grid-column: ${span.start} / ${span.end}; grid-row: ${row} / ${row + 1}; border-radius: 14px; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); padding: 16px 20px; transition: all 0.2s; border-left: 3px solid ${leftColor}; ${isMulti ? `border-right: 3px solid ${rightColor};` : ""} position: relative; overflow: hidden;"
